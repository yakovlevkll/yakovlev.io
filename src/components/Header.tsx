      ...
      >
        ...
        <nav className="mt-6">
          <ul className="-my-2 divide-y divide-zinc-100 text-base text-zinc-800 dark:divide-zinc-100/5 dark:text-zinc-300">
            {routes.map((el) => (
              <MobileNavItem href={el.path} key={el.path}>
                {el.title}
              </MobileNavItem>
            ))}
          </ul>
  ...
  return (
    <li>
      ...
      >
        {children}
        {isActive && (
          <span className="absolute inset-x-1 -bottom-px h-px bg-linear-to-r from-rose-500/0 via-rose-500/40 to-rose-500/0 dark:from-rose-400/0 dark:via-rose-400/40 dark:to-rose-400/0" />
        )}
      </Link>
    </li>
  )
  ...
  return (
    <nav {...props}>
      <ul className="flex rounded-full bg-white/90 px-3 text-sm font-medium text-zinc-800 shadow-lg ring-1 shadow-zinc-800/5 ring-zinc-900/5 backdrop-blur-sm dark:bg-zinc-800/90 dark:text-zinc-200 dark:ring-white/10">
        {routes.map((el) => (
          <NavItem href={el.path} key={el.path}>
            {el.title}
          </NavItem>
        ))}
      </ul>
...
function ThemeToggle() {
  ...
  return (
    ...
    >
      ...
      <MoonIcon className="hidden h-6 w-6 fill-zinc-700 stroke-zinc-500 transition not-[@media_(prefers-color-scheme:dark)]:fill-rose-400/10 not-[@media_(prefers-color-scheme:dark)]:stroke-rose-500 dark:block [@media(prefers-color-scheme:dark)]:group-hover:stroke-zinc-400" />
    </button>
  )
}
